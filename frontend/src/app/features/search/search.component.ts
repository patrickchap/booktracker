...
@Component({
  ...
  template: `
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div class="mb-8">
        <div class="mb-6">
          <h1 class="text-3xl font-display font-bold text-gray-900 dark:text-white tracking-tight">Search Books</h1>
          <p class="text-gray-500 dark:text-gray-400 mt-1">Explore millions of books to add to your library</p>
        </div>

        <form (submit)="search($event)" class="flex gap-3">
      ...
      @if (searchService.isLoading()) {
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          @for (book of searchService.searchResults().items; track book.googleBooksId; let i = $index) {
        ...
        <div class="text-center py-16 animate-fade-in">
          <div class="w-32 h-32 mx-auto bg-gradient-to-br from-gray-100 to-gray-50 dark:from-gray-800 dark:to-gray-700 rounded-full flex items-center justify-center mb-6">
            <svg class="w-16 h-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                    d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
            </svg>
          </div>
          <h3 class="text-xl font-display font-semibold text-gray-900 dark:text-white mb-2">No results found</h3>
          <p class="text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
            We couldn't find any books matching "{{ searchService.query() }}". Try a different search term.
          </p>
        </div>
      } @else {
        <div class="text-center py-16 animate-fade-in">
          ...
          <h3 class="text-xl font-display font-semibold text-gray-900 dark:text-white mb-2">Find your next read</h3>
          <p class="text-gray-500 dark:text-gray-400 max-w-sm mx-auto">
            Search millions of books by title, author, or ISBN to add to your library.
          </p>
        </div>
      }

      @if (notification()) {
        <div class="fixed bottom-4 right-4 bg-gradient-to-r from-green-600 to-green-500 text-white px-6 py-3 rounded-xl shadow-soft-lg animate-slide-up flex items-center gap-2">
